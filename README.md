...
- **읽음 기능 (내 서재에 책 추가 및 삭제)**
  ...
  - **작성**: ‘읽음’ 버튼 클릭시 POST 메서드로 `/api/library` 호출
    - **성공 시**: 모달로 "내서재에서 추가되었습니다." 메시지 후 북 카드에 ‘읽음’ 라벨 노출
  - **삭제**: 활성화된 ‘읽음’ 버튼 클릭시 DELETE 메서드로 `/api/library/${id}` 호출
    - **성공 시**: 모달로 "내서재에서 삭제되었습니다." 메시지 후 북 카드에 ‘읽음’ 라벨 제거
...
- **작성**: POST, `/api/library/review/${id}` 호출
- **수정**: PATCH, `/api/library/review/${id}` 호출
